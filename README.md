...
Semantic HTML has meaning to it so the browser will know this content is important on some scale

2. Name two big differences between ```display: block;``` and ```display: inline;```.
Display block will push everything around it while inline will only effect content in that same line not above or below.
...
It allow for every to create their own version of the program to work on for flexibility. Plus it give the project manager time to see if the code is not going ruin other code already entered.
